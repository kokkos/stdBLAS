...
{
  kokkos_matrix_inf_norm_test_impl(A_e0e1, static_cast<value_type>(0), false);
}

TEST_F(blas2_signed_float_fixture, kokkos_matrix_inf_norm_initvalue)
{
  kokkos_matrix_inf_norm_test_impl(A_e0e1, static_cast<value_type>(5.6), true);
}
...
{
  kokkos_matrix_inf_norm_test_impl(A_e0e1, static_cast<value_type>(5), true);
}
...
TEST_F(blas2_signed_complex_double_fixture, kokkos_matrix_inf_norm_noinitvalue)
{
  ...
  using stdc_t = value_type;
  if constexpr (alignof(value_type) == alignof(kc_t)){
    kokkos_matrix_inf_norm_test_impl(A_e0e1, static_cast<double>(0), false);
  }
}
...
TEST_F(blas2_signed_complex_double_fixture, kokkos_matrix_inf_norm_initvalue)
{
 namespace stdla = std::experimental::linalg;
  ...
  using stdc_t = value_type;
  if constexpr (alignof(value_type) == alignof(kc_t)){
    kokkos_matrix_inf_norm_test_impl(A_e0e1, static_cast<double>(5), true);
  }
}
