...
template<class Triangle, class A_t, class x_t, class y_t, class z_t>
void gold_solution_explicit_diagonal(Triangle /*t*/, A_t A, x_t x, y_t y, z_t z)
{

  if constexpr (std::is_same_v<Triangle, std::experimental::linalg::upper_triangle_t>)
  {
    for (std::size_t i=0; i<A.extent(0); ++i){
      typename y_t::value_type lsum{};
...
template<class Triangle, class A_t, class x_t, class y_t, class z_t>
void gold_solution_implicit_unit_diagonal(Triangle /*t*/, A_t A, x_t x, y_t y, z_t z)
{

  if constexpr (std::is_same_v<Triangle, std::experimental::linalg::upper_triangle_t>)
  {
    for (std::size_t i=0; i<A.extent(0); ++i){
      typename y_t::value_type lsum = x(i);
...
template<bool explicitDiagonal, class A_t, class Triangle, class x_t, class y_t, class z_t>
void kokkos_blas_updating_triangular_mat_vec_impl(A_t A, Triangle t, x_t x, y_t y, z_t z)
{
  ...
  if constexpr(explicitDiagonal){
    gold_solution_explicit_diagonal(t, A, x, y, z_gold);
    stdla::triangular_matrix_vector_product(KokkosKernelsSTD::kokkos_exec<>(),
    					    A, t, MyExplicitDiagonalTag(), x, y, z);
  }
  else{
    gold_solution_implicit_unit_diagonal(t, A, x, y, z_gold);
    stdla::triangular_matrix_vector_product(KokkosKernelsSTD::kokkos_exec<>(),
					    A, t, stdla::implicit_unit_diagonal_t(), x, y, z);
  }
  ...
  if constexpr(std::is_same_v<value_type, float>){
    ...
    for (std::size_t i=0; i<ext; ++i){
      ...
      // for z we need to use near or it won't work
...
#define TEST_TRI_MAX_VEC_A(TRIANGLEPART)				\
  ...
  kokkos_blas_updating_triangular_mat_vec_impl<explicitDiagonal>(A_sym_e0, \
								 stdla::TRIANGLEPART, \
								 x_e0, y_e0, z_e0); \
...
TEST_F(blas2_signed_float_fixture,
       kokkos_updating_triangular_matrix_vector_product_upper_explicit_diagonal)
{
  TEST_TRI_MAX_VEC_A(upper_triangle);
}

TEST_F(blas2_signed_float_fixture,
       kokkos_updating_triangular_matrix_vector_product_upper_implicit_unit_diagonal)
{
...
#ifdef USE_LOWER
TEST_F(blas2_signed_float_fixture,
       kokkos_updating_triangular_matrix_vector_product_lower_explicit_diagonal)
{
  TEST_TRI_MAX_VEC_A(lower_triangle);
}

TEST_F(blas2_signed_float_fixture,
       kokkos_updating_triangular_matrix_vector_product_lower_implicit_unit_diagonal)
{
